    ...
    try:
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow([
                ...
                result.duration_seconds # Write the raw float value
            ])
    except IOError as e:
...
def read_processed_files(filename: str) -> Dict[str, float]:
    """
    Reads the CSV, returning a dictionary mapping processed filepaths
    ...
    try:
        with open(filename, mode='r', newline='', encoding='utf-8') as csvfile:
            ...
            # Read rows and store filepath -> duration mapping
            for row_num, row in enumerate(reader, start=2): # start=2 for row number in file
                filepath = row.get('filepath')
                duration_str = row.get('duration_seconds')
                duration_float = 0.0 # Default duration if parsing fails
                ...
                if filepath:
                    if duration_str:
                        try:
                            ...
                            print(f"Warning: Could not parse duration '{duration_str}' for file '{filepath}' in {filename} (row {row_num}). Storing duration as 0.0.", file=sys.stderr)
                    else:
                         print(f"Warning: Missing duration for file '{filepath}' in {filename} (row {row_num}). Storing duration as 0.0.", file=sys.stderr)

                    # Store the filepath and its duration (or 0.0 if invalid)
                    processed_data[filepath] = duration_float

        print(f"Read {len(processed_data)} previously processed file entries from '{filename}'.")
        ...
        # Return potentially partially filled dict on other errors
...
def main():
    ...
    # --- Read previously processed files data (filepath -> duration) ---
    already_processed_data = read_processed_files(CSV_FILENAME)
    # Calculate previous total duration from the read data
    ...
    if found_files:
        ...
        for idx, sif_file_path in enumerate(found_files, start=1):
            ...
            if sif_file_path in already_processed_data:
                previous_duration = already_processed_data[sif_file_path]
                # Updated Skipping message to include previous duration
                print(f"---> [{counter_str}] Skipping (took {previous_duration:.1f} s): {sif_file_path}")
                skipped_count += 1
