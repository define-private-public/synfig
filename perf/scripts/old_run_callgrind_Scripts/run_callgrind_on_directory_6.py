...
def append_result_to_csv(result: ProcessingResult, filename: str):
    ...
    try:
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header only if the file didn't exist before opening
...
def main():
    ...
    if found_files:
        ...
        for sif_file_path in found_files:
            ...
            try:
                ...
                result = subprocess.run(
                    [executable_abs_path, sif_file_path], # Defined command here
                    check=False,
        ...
        fail_count = total_files - success_count # All non-Success states are failures

        print(f"Total .sif files found: {len(found_files)}") # Keep original found count
        print(f"Attempted processing: {total_files}")
