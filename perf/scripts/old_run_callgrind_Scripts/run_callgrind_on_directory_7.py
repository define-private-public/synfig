...
def append_result_to_csv(result: ProcessingResult, filename: str):
    ...
    try:
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header only if the file didn't exist before opening or is empty
            # Check size in case file exists but is empty
...
def main():
    ...
    collecting results, and logging results incrementally to CSV.
    """
    ...
    results_list: List[ProcessingResult] = [] # Initialize list to store results
    if found_files:
        ...
        # Recalculate counts based on the final list (though they should match loop counts)
        success_count = sum(1 for r in results_list if r.status == "Success")
        fail_count = total_files_processed - success_count # All non-Success states are failures

        print(f"Total .sif files found: {len(found_files)}") # Keep original found count
        print(f"Attempted processing: {total_files_processed}")
        print(f"Succeeded (exit code 0): {success_count}")
        print(f"Failed (non-zero exit or error): {fail_count}")
