...
studio::render_color_to_window(const Cairo::RefPtr<Cairo::Context> &cr, const Gdk::Rectangle &ca, const synfig::Color &color)
{
	...
	if(color.get_alpha()!=1.0)
	{
		...
		const Color blend1 = Color::blend(color,Color(0.75, 0.75, 0.75),1.0);
		...
		const Color blend2 = Color::blend(color,Color(0.5, 0.5, 0.5),1.0);
	...
	{
		synfig::Color c = gamma.apply(clamped(
			color.get_r(),
			color.get_g(),
			color.get_b(),
			color.get_a()
